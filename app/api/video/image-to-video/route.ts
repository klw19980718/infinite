...
import { uploadFileToWaveSpeed, submitImageToVideoTask, submitImageToVideoFastTask } from "@/lib/wavespeed"
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Use NGROK_DEV_URL for local development only, otherwise use NEXT_PUBLIC_SITE_URL
      // In production, NGROK_DEV_URL should not be set
      const baseUrl = process.env.NODE_ENV === 'development' && process.env.NGROK_DEV_URL
        ? process.env.NGROK_DEV_URL
